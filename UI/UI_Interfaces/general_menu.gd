extends CanvasLayer
@onready var inventario: NinePatchRect = $Control/Inventario
@onready var animation_player: AnimationPlayer = $AnimationPlayer
@onready var bestiary: NinePatchRect = $Control/Bestiary
@onready var menu: NinePatchRect = $Control/Menu
...
func _ready() -> void:
	...
	$Control/Saving_Alert.visible = false 
	player = self.owner
	Dialogic.signal_event.connect(_on_Dialogos)
	

func _on_Dialogos(Argumento : String):
	if Argumento == "Abrir_Inventario":
...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("Esc") :
		if  menu.visible : 
			...
			$Control/Bestiary.visible = false
			get_tree().paused = false 
			
		elif !menu.visible and !Dialogic.is_active : 
	...
	if event.is_action_pressed('Inventory'):
		if inventario.visible :
			animation_player.play("Hide_inventory")
			get_tree().paused = false 
		elif !$Control/Bestiary.visible :
			if !Dialogic.is_active:
				animation_player.play("Show_inventory")
				get_tree().paused = true
...
func _on_guardar_pressed() -> void:
	...
	$Control/Saving_Alert.visible = true
